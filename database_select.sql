...
-- Mon Oct  1 18:53:01 2018
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

CREATE SCHEMA IF NOT EXISTS `departamentos_municipios` DEFAULT CHARACTER SET utf8 ;
USE `departamentos_municipios` ;

CREATE TABLE IF NOT EXISTS `departamentos_municipios`.`departamentos` (
...
CREATE TABLE IF NOT EXISTS `departamentos_municipios`.`municipios` (
  `id_municipio` INT NOT NULL AUTO_INCREMENT,
  `municipio` VARCHAR(150) NULL,
  `estado` INT NULL,
  `departamento_id` INT NULL,
  PRIMARY KEY (`id_municipio`),
  INDEX `llave_departamento_idx` (`departamento_id` ASC),
  CONSTRAINT `llave_departamento`
    FOREIGN KEY (`departamento_id`)
    REFERENCES `departamentos_municipios`.`departamentos` (`id_departamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
